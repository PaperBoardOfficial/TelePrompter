...
export default function PromptTemplates() {
  ...
  useEffect(() => {
    const loadTemplates = async () => {
      try {
        ...
        showToast('Error', 'Failed to load prompt templates', 'error');
        setLoading(false);
      }
    };
  ...
  const handleSaveTemplates = async () => {
    try {
      ...
      if (result.success) {
        showToast('Success', 'Prompt templates saved successfully', 'success');
        setIsEditing(false);
        setEditingTemplate(null);
      } else {
        showToast('Error', result.error || 'Failed to save templates', 'error');
      }
    } catch (error) {
      showToast('Error', 'Failed to save prompt templates', 'error');
    }
  };
