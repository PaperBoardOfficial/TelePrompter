...
export class IpcHandler {
  ...
  private processingHelper: ProcessingService;
  private appState: AppState;

  constructor(
    windowService: WindowService,
    screenshotHelper: ScreenshotService,
    processingHelper: ProcessingService,
    appState: AppState
  ) {
  ...
  public initialize(): void {
    ...
    ipcMain.handle(
      ...
      async (event, { width, height }: { width: number; height: number }) => {
        if (width && height) {
          this.windowService.setDimensions(width, height);
        }
      }
    );
    ...
    ipcMain.handle(
      ...
      async (_, { templates, activeTemplate }) => {
        try {
