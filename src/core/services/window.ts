...
/**
 * WindowService handles all operations related to the application window
...
export class WindowService {
  ...
  public moveRight(): void {
    const screenDimensions = this.screenDimensions;
    if (!screenDimensions) return;
    this.moveHorizontal((x) =>
      Math.min(
        screenDimensions.width - (this.size?.width || 0) / 2,
        x + this.moveStep
      )
    );
  }
  ...
  private moveVertical(updateFn: (y: number) => number): void {
    if (!this.mainWindow || !this.position || !this.screenDimensions) return;
